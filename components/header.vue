<template>
    <div>
        ...
        <v-app-bar @mouseout="closeMenu" scroll-behavior="hide elevate" scroll-threshold="290" :class="`${ showHero ? 'show' : ''} ${ color }`" id="app-header">
            <v-row class="menu">
                <v-spacer></v-spacer>
...
<script setup>
    ...
    const handleDesktopMenuItemClick = (e, item) => {
        if(item?.submenu){
            e.preventDefault()

            if(toRaw(selectedItem.value) === item)
                openSubmenu.value = true
            else {
                if(openSubmenu.value === false)
                    openSubmenu.value = true

                selectedItem.value = toValue(item)
            }
        } else {
